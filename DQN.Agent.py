...
class DQNAgent:
    ...
    def select_action(self, state, valid_actions):
        ...
        # Convert (6,7) → (42,)
        if state.shape == (self.nrow, self.ncol):
            state = state.flatten()
    ...
    def train(self):
        ...
        # includes valid_actions
        batch = random.sample(self.replay_buffer.buffer, self.batch_size)
        states, actions, rewards, next_states, dones, valid_actions_batch = zip(*batch)
        ...
        with torch.no_grad():
            ...
            #  mask invalid actions
            masked_next_q = torch.full_like(next_q_values, -1e9)

            for i, valid_actions in enumerate(valid_actions_batch):
                masked_next_q[i, valid_actions] = next_q_values[i, valid_actions]

            max_next_q = masked_next_q.max(1, keepdim=True)[0]
